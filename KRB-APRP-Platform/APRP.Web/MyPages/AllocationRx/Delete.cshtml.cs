...
namespace APRP.Web.MyPages.AllocationRx
{
    ...
    public class DeleteModel : PageModel
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1031:Do not catch general exception types", Justification = "<Pending>")]
        [Authorize(Claims.Permission.Allocation.Delete)]
        public async Task<IActionResult> OnPostAsync(long? id)
        {
            try
            {
                ...
                var _allocationResp = await _allocationService.RemoveAsync(myid).ConfigureAwait(false);
                Allocation = _allocationResp.Allocation;
                if (Referer != null)
                {
            ...
            {
                _logger.LogError(Ex, $"AllocationRx.Delete Page Error: {Ex.Message} " +
                    $"{Environment.NewLine}");
                return Page();
            }
        }
