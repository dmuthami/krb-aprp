...
namespace APRP.Web.MyPages.WorkCategoryFundingMatrixRx
{
    ...
    public class DeleteModel : PageModel
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1031:Do not catch general exception types", Justification = "<Pending>")]
        [Authorize(Claims.Permission.WorkCategoryFundingMatrix.Delete)]
        public async Task<IActionResult> OnPostAsync(long? id)
        {
            try
            {
                ...
                var workCategoryFundingMatrixResp = await _workCategoryFundingMatrixService.RemoveAsync(myid).ConfigureAwait(false);

                return RedirectToPage("./Index");
            }
            catch (Exception Ex)
            {
                _logger.LogError(Ex, $"WorkCategoryFundingMatrixRx.Delete Page Error: {Ex.Message} " +
                    $"{Environment.NewLine}");
                return Page();
            }
        }
