...
namespace APRP.Web.MyPages.WorkCategoryFundingMatrixRx
{
    ...
    public class CreateModel : PageModel
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1031:Do not catch general exception types", Justification = "<Pending>")]
        [Authorize(Claims.Permission.WorkCategoryFundingMatrix.Add)]
        public async Task<IActionResult> OnPostAsync()
        {
            try
            {
                ...
                var revenueCollectionResp = await _workCategoryFundingMatrixService.FindByAuthorityAndFinancialIdAsync(WorkCategoryFundingMatrix.AuthorityId, WorkCategoryFundingMatrix.FinancialYearId).ConfigureAwait(false);
                ...
                {
                    var workCategoryFundingMatrixResp = await _workCategoryFundingMatrixService.AddAsync(WorkCategoryFundingMatrix).ConfigureAwait(false);
                }
                return RedirectToPage("./Index");
            }
            catch (Exception Ex)
            {
                _logger.LogError(Ex, $"WorkCategoryFundingMatrixRx.Add Page Error: {Ex.Message} " +
                    $"{Environment.NewLine}");
                return Page();
            }
        }
