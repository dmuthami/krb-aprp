    ...
    public class EditModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(long? id)
        {
            try
            {
                ...
                var allocationCodeUnitResp = await _allocationCodeUnitService.FindByIdAsync(myid).ConfigureAwait(false);
                AllocationCodeUnit = allocationCodeUnitResp.AllocationCodeUnit;

                if (AllocationCodeUnit == null)
                {
                    return NotFound();
                }
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1031:Do not catch general exception types", Justification = "<Pending>")]
        [Authorize(Claims.Permission.AllocationCodeUnit.Change)]
        public async Task<IActionResult> OnPostAsync()
        {
            try
            {
                ...
                var allocationCodeUnitResp = await _allocationCodeUnitService.Update(AllocationCodeUnit.ID, AllocationCodeUnit).ConfigureAwait(false);
            ...
            {
                _logger.LogError(Ex, $"AllocationCodeUnitRx.Edit Page Error: {Ex.Message} " +
                    $"{Environment.NewLine}");
                return Page();
            }
        }
