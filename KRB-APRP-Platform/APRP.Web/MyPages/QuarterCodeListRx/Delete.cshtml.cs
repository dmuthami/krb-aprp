...
namespace APRP.Web.MyPages.QuarterCodeListRx
{
    ...
    public class DeleteModel : PageModel
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1031:Do not catch general exception types", Justification = "<Pending>")]
        [Authorize(Claims.Permission.QuarterCodeList.Delete)]
        public async Task<IActionResult> OnPostAsync(long? id)
        {
            try
            {
                ...
                var quarterCodeListResp = await _quarterCodeListService.RemoveAsync(myid).ConfigureAwait(false);

                return RedirectToPage("./Index");
            }
            catch (Exception Ex)
            {
                _logger.LogError(Ex, $"QuarterCodeListRx.Delete Page Error: {Ex.Message} " +
                    $"{Environment.NewLine}");
                return Page();
            }
        }
