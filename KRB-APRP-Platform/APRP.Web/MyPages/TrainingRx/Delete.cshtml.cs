...
namespace APRP.Web.MyPages.TrainingRx
{
    ...
    public class DeleteModel : PageModel
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1031:Do not catch general exception types", Justification = "<Pending>")]
        [Authorize(Claims.Permission.Training.Delete)]
        public async Task<IActionResult> OnPostAsync(long? id)
        {
            try
            {
                ...
                var _trainingResp = await _trainingService.RemoveAsync(myid).ConfigureAwait(false);
                Training = _trainingResp.Training;
                if (Referer != null)
                {
            ...
            {
                _logger.LogError(Ex, $"TrainingRx.Delete Page Error: {Ex.Message} " +
                    $"{Environment.NewLine}");
                return Page();
            }
        }
