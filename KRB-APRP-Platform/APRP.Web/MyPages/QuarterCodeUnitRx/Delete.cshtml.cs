...
namespace APRP.Web.MyPages.QuarterCodeUnitRx
{
    ...
    public class DeleteModel : PageModel
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1031:Do not catch general exception types", Justification = "<Pending>")]
        [Authorize(Claims.Permission.QuarterCodeUnit.Delete)]
        public async Task<IActionResult> OnPostAsync(long? id)
        {
            try
            {
                ...
                var _quarterCodeUnitResp = await _quarterCodeUnitService.RemoveAsync(myid).ConfigureAwait(false);
                QuarterCodeUnit = _quarterCodeUnitResp.QuarterCodeUnit;
                if (Referer != null)
                {
            ...
            {
                _logger.LogError(Ex, $"QuarterCodeUnitRx.Delete Page Error: {Ex.Message} " +
                    $"{Environment.NewLine}");
                return Page();
            }
        }
